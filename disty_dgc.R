...
setwd("C:/jobb/disty/688244-144687")

MyRivernetwork <- line2network(path=".", layer="688244-144687_rivers", tolerance=5)
# 
# plot(MyRivernetwork)
# 
...
sites <- pointshp2segvert(path=".", layer="688244-144687_sites", rivers=MyR3)
...
dams <- pointshp2segvert(path=".", layer="688244-144687_dams", rivers=MyR3)
...
#dev.copy2pdf(file="688244-144687_map.pdf", width=10, height=7)
...
### I want to add fragment ID to a site table and save it out ... 
sites2 <- sites
sites2$id2 <- as.character(1:nrow(sites))
str(fragment_table)
str(sites2)
sites2$id2 <- as.integer(sites2$id2)
sites2 <- sites2 %>% left_join(fragment_table, "fragmentID", by=c("id2"="site"))
sites2$river_id <- "688244-144687"

write.table(sites2, file=paste0("C:/jobb/disty/out_files/f_tables/f_table_688244-144687_",Sys.Date(),".csv"), row.names=F)


save.image(file=paste0("C:/jobb/disty/688244-144687/688244-144687_image_",Sys.Date(),".RData"))
...
with(megakey, plot(rho_trout ~ trout_prop_unpaired, cex = n_years))
with(megakey, plot(rho_minnow ~ minnow_prop_unpaired, cex = n_years))
with(megakey, plot(rho_roach ~ roach_prop_unpaired, cex = n_years))
with(megakey, plot(rho_perch ~ perch_prop_unpaired, cex = n_years))
with(megakey, plot(rho_pike ~ pike_prop_unpaired, cex = n_years))
...
plot(data=megakey, rho_trout ~ trout_avg, col=(as.numeric(megakey$flowconn)+1))
summary(m1 <- lm(data=megakey, rho_trout ~ trout_avg + flowconn01 * rivdist * over_frag01))
...
megakey$river_id <- "688244-144687"
...
write.table(megakey, file=paste0("C:/jobb/disty/out_files/megakeys/mega_key_688244-144687_",Sys.Date(),".csv"))
...
write.table(portfolio, file="C:/jobb/disty/out_files/688244-144687.csv", row.names = F, sep=";", dec=".")
...
write.table(portfolio2, file="C:/jobb/disty/out_files/occ/688244-144687.csv", row.names = F, sep=";", dec=".")
